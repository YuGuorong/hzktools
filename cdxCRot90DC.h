...
private:
	...
	public:
		long X(long x, long y) const 
		{ 
			return x*m_iCos + y*m_iSin;
		}
		long Y(long x, long y) const 
		{ 
			return y*m_iCos - x*m_iSin; 
		}
		CSize Size(const CSize & sz) const 
		{ 
			if(!m_iCos) 
				return sz; 
			return CSize(sz.cy,sz.cx); 
		}
	};

private:
	CDC		*m_pDC;
	CRect	m_rectMine,m_rectMineClipped,m_rectDC,m_rectDCClipped;
	Angle	m_Angle;
	CBitmap	m_Bitmap;
	CSize	m_szBitmap;
	bool	m_bFinished;
...
public:
	...
	cdxCRot90DC(CDC & destDC, const CRect & rectDC,int iAngle,bool bCopy=false): m_pDC(NULL), m_bFinished(true) 
	{
		Create(destDC,rectDC,iAngle,bCopy);
	}
	cdxCRot90DC(CDC *pDC, const CRect & rectDC, int iAngle, bool bCopy = false) : m_pDC(NULL), m_bFinished(true)
	{ 
		Create(pDC,rectDC,iAngle,bCopy); 
	}
	cdxCRot90DC(CDC & destDC, int iAngle, bool bCopy = false) : m_pDC(NULL), m_bFinished(true) 
	{ 
		VERIFY( Create(destDC,iAngle,bCopy) ); 
	}
	cdxCRot90DC(CDC *pDC, int iAngle, bool bCopy = false) : m_pDC(NULL), m_bFinished(true) 
	{ 
	...
	{ 
		m_bFinished = true; 
		if(m_pDC && m_hDC) 
			IntersectClipRect(CRect(0,0,0,0)); 
	}
	void Destroy();
	...
	{ 
		rotate(pPnts,nCnt,mkAngle(iAngle)); 
	}
	...
	void DrawControl(CString strText, 
		HICON hIcon, 
		UINT nDrawTextFlags = DT_CENTER|DT_VCENTER, 
		UINT nMyFlags = 0, 
		UINT nIconDist = 2) 
	{ 
		DrawControl(m_rectMine,strText,hIcon,nDrawTextFlags,nMyFlags,nIconDist); 
	}
	...
	void DrawControl(CString strText, 
		...
		UINT nIconDist = 2)  
	{ 
		DrawControl(m_rectMine,strText,NULL,nDrawTextFlags,nMyFlags,nIconDist); 
	}

	void DrawControl(CRect rect, 
		CString strText, 
		HICON hIcon, 
		UINT nDrawTextFlags = DT_CENTER|DT_VCENTER,
		UINT nMyFlags = 0, 
	...
	void DrawControl(CRect rect, CString strText, UINT nDrawTextFlags = DT_CENTER|DT_VCENTER, UINT nMyFlags = 0, UINT nIconDist = 2) 
	{ 
		DrawControl(rect,strText,NULL,nDrawTextFlags,nMyFlags,nIconDist); 
	}
